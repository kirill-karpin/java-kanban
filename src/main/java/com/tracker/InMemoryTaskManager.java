...
public class InMemoryTaskManager implements TaskManager {
    ...
    private boolean validateOverlapTask(Task task) {
        ...
        Optional<Task> overlappingTask = prioritizedTasks
                .stream()
                .filter(t -> t.isOverlap(task)).findFirst();
    ...
    private Status getEpicStatus(Epic epic) {
        ...
        if (tasksId.stream().map(subTasks::get)
                .anyMatch(task -> task.getStatus() == Status.IN_PROGRESS || task.getStatus() == Status.NEW)) {
    ...
    private Optional<LocalDateTime> getEpicStartTime(Epic epic) {
        ...
        LocalDateTime result = subTasks.values().stream()
    ...
    private Duration getEpicDuration(Epic epic) {
        ...
        Optional<Duration> result = subTasks.values().stream()
                .filter(task -> task.getEpicId() != null
                        && task.getEpicId() == epic.getId() && task.getDuration() != null)
    ...
    public List<SubTask> getEpicSubtasks(int id) {
        return subTasks.values().stream().filter(
                task -> task.getEpicId() != null && task.getEpicId() == id).toList();
    }
    ...
    public void addEpicSubTask(int epicId, SubTask subTask) {
        ...
        if (epic != null) {
            ...
            startTime.ifPresent(localDateTime -> epic.setStartTime(localDateTime.plus(epic.getDuration())));
